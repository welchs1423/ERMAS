...
export default function App() {
  ...
  const handleRefresh = () => {
    if (currentNickname) fetchStats(currentNickname);
  };
    ...
    return (
      <div className="min-h-screen bg-[#f0eff0] flex items-center justify-center">
        <div className="bg-white rounded-2xl shadow-lg p-10 w-full max-w-md">
          <h1 className="text-center text-3xl font-extrabold text-gray-800 mb-1 tracking-tight">
            ERMAS
          </h1>
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleSearch(inputValue); }}>
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition-colors"
                autoFocus
              />
              <button
                type="submit"
                className="px-5 py-2.5 bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium rounded-lg transition-colors flex items-center gap-1.5"
              >
                <Search size={14} />
                검색
              </button>
